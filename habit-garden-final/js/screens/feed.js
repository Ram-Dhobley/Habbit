...
HG.screens.feed = {
  ...
  render() {
    ...
    ${this.renderLbBar()}
    <div id="feed-items">${this.renderPosts()}</div>
    <div style="height:8px"></div>`;
  },

  renderLbBar() {
    const chips = ['running','meditation','reading','cycling','walking','city','india','global'];
    const labels = {'running':'🏃 Running','meditation':'🧘 Meditation','reading':'📚 Reading','cycling':'🚴 Cycling','walking':'🚶 Walking','city':'📍 City','india':'🇮🇳 India','global':'🌍 Global'};
    const lbData = HG.data.leaderboards[this.activeLb] || HG.data.leaderboards.running;
    return `
    <div class="lb-bar">
      <div class="lb-bh">
      ...
      <div class="lb-chips">
        ${chips.map(c => `<div class="lbc${c===this.activeLb?' on':''}" onclick="HG.screens.feed.setLb('${c}')">${labels[c]}</div>`).join('')}
      </div>
      <div class="lb-pod" id="lb-pod">
        ${lbData.map(r=>`<div class="lb-p"><span style="font-size:15px">${r[0]}</span><div><div class="lb-pn">${r[1]}</div><div class="lb-pv">${r[2]}</div></div></div>`).join('')}
      </div>
    </div>`;
  },

  renderPosts() {
    return HG.data.feedPeers.map(p => this.renderPost(p)).join('');
  },

  renderPost(p) {
    const liked = HG.state.likedPosts.includes(p.id);
    return `
    <div class="fi" id="fi-${p.id}">
      ${p.pinned ? `<div class="fi-pinned-badge">📌 Pinned</div>` : `<div class="fi-expiry">${p.expiry}</div>`}
      <div class="fi-head">
      ...
      <div class="fi-acts">
        ...
        <div class="fi-icon-btn" onclick="HG.util.sharePost('${p.id}')" style="margin-left:auto">
  ...
  switchTab(i) {
    ...
    if (title) title.textContent = i===1 ? '🏆 Discover · Public preset habits only' : '🏆 Leaderboard · preset habits only';
    if (i===1) HG.util.toast('Showing public preset habits from everyone');
  },

  setLb(type) {
    this.activeLb = type;
    document.querySelectorAll('.lb-chips .lbc').forEach(c => c.classList.remove('on'));
    document.querySelectorAll('.lb-chips .lbc').forEach(c => {
      if (c.textContent.toLowerCase().includes(type) || c.getAttribute('onclick').includes(type)) c.classList.add('on');
    });
    const data = HG.data.leaderboards[type] || HG.data.leaderboards.running;
    const pod = document.getElementById('lb-pod');
    if (pod) pod.innerHTML = data.map(r=>`<div class="lb-p"><span style="font-size:15px">${r[0]}</span><div><div class="lb-pn">${r[1]}</div><div class="lb-pv">${r[2]}</div></div></div>`).join('');
  },

  toggleLike(id) {
    const idx = HG.state.likedPosts.indexOf(id);
    const post = HG.data.feedPeers.find(p => p.id === id);
    const btn = document.getElementById(`like-${id}`);
    if (!btn || !post) return;
    if (idx > -1) {
      HG.state.likedPosts.splice(idx, 1);
      btn.classList.remove('lk');
      btn.innerHTML = `🤍 ${post.likes}`;
    } else {
      HG.state.likedPosts.push(id);
