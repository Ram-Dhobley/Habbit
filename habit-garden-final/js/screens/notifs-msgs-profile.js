...
HG.screens.notifs = {
  render() {
    ...
    const dismissed = HG.state.dismissedReqs;
    return `
    ...
    ${requests.filter(r => !dismissed.includes(r.id)).map(r => this.renderReq(r)).join('')}
    ...
    <div style="background:var(--surf);margin:0 18px;border-radius:14px;border:0.5px solid var(--bdr);overflow:hidden">
      ${activity.map((a,i) => this.renderActivity(a,i,activity.length)).join('')}
    </div>
  ...
  renderActivity(a, i, total) {
    return `
    <div class="act-notif${a.unread?' unread':''}" style="${i===total-1?'border:none':''}">
      <div class="an-av" style="background:${a.avatarBg}">${a.initials}${a.ico?`<div class="an-ico" style="background:${a.icoBg}">${a.ico}</div>`:''}</div>
  ...
  accept(id, msg) {
    HG.state.dismissedReqs.push(id);
    HG.state.save();
...
HG.screens.messages = {
  render() {
    const { messages, messageRequests } = HG.data;
    return `
    <div class="screen-hdr">
      <div class="screen-hl">Messages</div>
      <div class="hdr-icon" onclick="HG.util.toast('New message coming soon')">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 20h9M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
      </div>
    </div>
    <div class="msearch">
    ...
    <div class="conv-list">
      ${messages.map(m => this.renderConv(m)).join('')}
    </div>
    <div class="msg-section-label" style="margin-top:9px">Message requests</div>
    <div class="msg-req-banner" onclick="HG.util.toast('${messageRequests.length} message requests from non-peers')">
      <div class="req-av-stack">
        ${messageRequests.map(r => `<span style="background:${r.bg}">${r.initials}</span>`).join('')}
      </div>
      <div style="flex:1">
        <div style="font-size:13px;font-weight:500">${messageRequests.length} message requests</div>
        <div style="font-size:11.5px;color:var(--tx3);margin-top:1px">Not your peers yet</div>
      </div>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="color:var(--tx3)"><polyline points="9 18 15 12 9 6"/></svg>
    </div>
    <div style="height:16px"></div>`;
  },

  renderConv(m) {
    const isTrybe = m.type === 'trybe';
    const tagStyle = isTrybe
      ? 'color:var(--amber);background:var(--amber-l)'
      : 'color:var(--tx3);background:var(--surf2)';
    return `
    <div class="conv" onclick="HG.util.toast('Chat: ${m.name} (coming soon)')">
      <div class="conv-av" style="background:${m.avatarBg}">
        ${m.initials}
        ${m.online ? '<div class="conv-on"></div>' : ''}
        ${isTrybe ? `<div class="conv-gbadge">${m.members}</div>` : ''}
      </div>
      <div style="flex:1;min-width:0">
        <div class="conv-name">${m.name} <span class="ctag" style="${tagStyle}">${isTrybe ? 'Trybe' : 'Peer'}</span></div>
  ...
  render() {
    ...
    <div class="prof-top-row">
      <div class="prof-av-wrap">
        <div class="prof-av" onclick="HG.util.toast('Change photo')">${user.name.split(' ').map(w=>w[0]).join('')}</div>
        <div class="prof-av-badge" onclick="HG.util.toast('Change photo')">
      ...
      <div class="prof-btns">
        ...
        <div class="pa-ico" onclick="HG.util.toast('Settings')">
    ...
    <div class="prof-info">
      ...
      <div class="prof-handle"><span>${user.handle}</span><span style="width:3px;height:3px;border-radius:50%;background:var(--tx3);display:inline-block"></span><span>${user.bio}</span></div>
      <div class="prof-loc"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg> ${user.city} · ${user.country}</div>
    ...
    <div class="plant-scroll">
      ${plantIds.map(id => `
        <div class="mgi" onclick="HG.screens.plantDetail.open('${id}')">
          <div style="font-size:27px">${plants[id].emoji}</div>
          <p>${plants[id].name.split(' ')[0]}</p>
          <p style="font-size:9px;color:var(--g600);margin-top:1px">🔥 ${plants[id].streak}d</p>
        </div>`).join('')}
      <div class="mgi" style="opacity:.48" onclick="HG.util.toast('Upgrade to Premium for more plants')">
    ...
    <div class="pinned-grid">
      ${pinnedMoments.map(m => `
        <div class="pg-item" style="background:${m.bg}" onclick="HG.util.toast('${m.label}')">
          ...
          <div class="pg-share" onclick="event.stopPropagation();HG.util.toast('Shared!')">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
          </div>
        </div>`).join('')}
      <div class="pg-item" style="background:var(--surf2);border:0.5px dashed var(--bdr)" onclick="HG.util.toast('Pin a moment from your feed')">
    ...
    <div style="margin:0 18px 20px;background:linear-gradient(135deg,#4527a0,#7c3aed);border-radius:14px;padding:14px;display:flex;align-items:center;gap:12px">
      ...
      <div><div style="font-size:13.5px;font-weight:600;color:#fff">Upgrade to Premium</div><div style="font-size:11.5px;color:rgba(255,255,255,.75);margin-top:2px">Unlimited plants · Pro Tracks</div></div>
      <button onclick="HG.util.toast('₹299/mo · Opening plans…')" style="margin-left:auto;background:rgba(255,255,255,.2);color:#fff;border:none;border-radius:8px;padding:7px 12px;font-size:12px;font-weight:500;cursor:pointer;font-family:var(--fn)">Upgrade</button>
    </div>`;
  }
};
